...
class MCTSNode:
        ...
        while exploration_depth < max_exploration_depth:
            ...
            if not current.is_expanded:
                if TreeEnv.data_all is not None:
                    ...
                    current.select_expand_action(dim_per_split=dim_per_split,
                                                 reference_data=TreeEnv.data_all,
                                                 original_var=original_var,
                                                 add_estimate=True,
                                                 apply_global_variance=True)
                    end_time = time.time()
            ...
            child_action_score_all = np.ones([len(current.state), self.n_actions_types, len(TreeEnv.data_all)]) * float(
                '-inf')
    ...
    def select_expand_action(self, dim_per_split,
                             reference_data,
                             original_var,
                             add_estimate=False,
        ...
        :param dim_per_split: the number of splits for each latent features
                        ...
                        if len(check_split_state_pair) == 0:
                            ...
                            for state_list_str in check_split_state_pair_new.keys():
                                if check_split_state_pair.get(state_list_str) is not None:  # remove duplicate splits
                                    ...
                                    check_split_state_pair.update(
                                        {state_list_str: check_split_state_pair_return[state_list_str]})
    ...
    def print_tree(self, TreeEnv, writer=None, print_indent=0):
        ...
        node_string += "Level{0}|Node: action={1}, N={2}, Q={3}, U={4}, return={5}|".format(self.level, self.action,
                                                                                            self.N,
                                                                                            round(self.Q, 6),
                                                                                            round(self.U, 6),
...
class MCTS:
    ...
    def tree_search(self, k, original_var, avg_timer_record, TreeEnv, log_file, process, pid, apply_rollout=False):
        ...
        leaves = []  # the leaf selected by each simulation
    ...
    def take_move(self, TreeEnv, log_file, process, round_counter, saved_nodes_dir):
        ...
        print("Clean total memory usage is {0} at {1}".format(mme_a, datetime.now().strftime('%Y-%m-%d %H:%M:%S')),
              file=log_file)
        ...
        mme_a = float(process.memory_info().rss) / 1000000
        print("Deep copy total memory usage is {0} at {1}".format(mme_a, datetime.now().strftime('%Y-%m-%d %H:%M:%S')),
              file=log_file)
...
def execute_episode_single(num_simulations, TreeEnv, tree_writer,
                           ...
                           init_var_list, ignored_dim,
                           shell_round_number, shell_saved_model_dir, log_file,
                           disentangler_name,
                           apply_split_parallel=False, save_gap=2, play=None):
    pid = os.getpid()
    process = psutil.Process(pid)  # monitor the usage of memory

    next_end_flag = False
    if play is None:
        simulations_per_round = 200
    else:
        simulations_per_round = play
    if apply_split_parallel:  # apply multi-process to accelerate learning
        global SPLIT_POOL
        global PROCESS_NUMBER
        PROCESS_NUMBER = max_k - len(ignored_dim)
    ...
    k = 5  # the number of sorted features to check, k<max_k
    global c_PUCT
    initial_c_puct = str(c_PUCT).replace('.', '_')

    c_puct_step_size = float(c_PUCT) / (num_simulations / simulations_per_round)

    if shell_round_number is None:  # we initialize the mcts
        round_counter = 0
    ...
    else:  # we read the previously saved mcts model
        ...
        if round_counter > 0:
            ...
            mcts.initialize_search(random_seed=0, init_state=init_state, init_var_list=init_var_list,
                                   n_action_types=TreeEnv.n_action_types, ignored_dim=ignored_dim)
            saved_nodes_dir = mcts_saved_dir.replace('tree_models', 'tree_nodes') \
                              + '_CPUCT' + str(initial_c_puct) + '_DEG' + disentangler_name + '_' + \
