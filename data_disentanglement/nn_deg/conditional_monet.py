...
class CMONetModel():
    ...
    def __init__(self, num_slots, z_dim, condition_size, lr, device, input_nc=3, beta=0.5, gamma=0.5):
        ...
        self.netAttn = Attention(input_nc=self.input_nc, output_nc=1).to(self.device)
        init_weights(self.netAttn)
        self.netCVAE = ConditionalComponentVAE(input_nc=self.input_nc,
    ...
    def forward(self, x, condition):
        ...
        for k in range(self.num_slots):
            ...
            m_tilde_k_logits, x_mu_k, x_logvar_k, z_mu_post_k, z_logvar_post_k, z_mu_prior_k, z_logvar_prior_k \
                = self.netCVAE(x, log_m_k, condition, k == 0)
            x_mu.append(x_mu_k)
            z_mu.append(z_mu_post_k)

            # KLD is additive for independent distributions
            # self.loss_E += -0.5 * (1 + z_logvar_post_k - z_mu_post_k.pow(2) - z_logvar_post_k.exp()).sum()

            # KLD between two Gaussian
            # print(z_logvar_prior_k.size())
            # print(z_logvar_post_k.size())
            self.loss_E += (z_logvar_prior_k - z_logvar_post_k + \
                           (z_logvar_post_k.exp() + torch.square(z_mu_post_k - z_mu_prior_k))/(2*z_logvar_prior_k.exp())).sum()
    ...
    def optimize_parameters(self, x, conditions):
        """Update network weights; it will be called in every training iteration."""
        self.forward(x, conditions)  # first call forward to calculate intermediate results
...
class ConditionalComponentVAE(nn.Module):

    def __init__(self, input_nc, condition_size, z_dim=16, full_res=True):
        super().__init__()
        ...
        self.conditional_q_nn = nn.Sequential(nn.Linear(z_dim * 4, 128),
                                              nn.LeakyReLU(0.2, True),
                                              nn.Linear(128, 64),
                                              nn.LeakyReLU(0.2, True),
    ...
    def forward(self, x, log_m_k, condition, background=False):
        """
        :param x: Input image
        ...
        params_prior = self.conditional_prior(condition)
        z_mu_prior = params_prior[:, :self._z_dim]
        z_logvar_prior = params_prior[:, self._z_dim:]

        params_post = self.encoder(torch.cat((x, log_m_k), dim=1))
        params_post = self.conditional_q_nn(torch.cat([params_post, self.condition_encoder(condition)], dim=1))
        z_mu_post = params_post[:, :self._z_dim]
        z_logvar_post = params_post[:, self._z_dim:]
        ...
        return m_logits, x_mu, x_logvar, z_mu_post, z_logvar_post, z_mu_prior, z_logvar_prior
